...
export function AiCompanionsPage() {
  ...
  useEffect(() => {
    if (!selectedId || detail?.visualIdentity?.status !== "review") { setVisualReferenceUrls([]); return; }
    let cancelled = false;
    const urls: string[] = [];
    Promise.all(["canonical", "three-quarter", "side"].map((view) => fetchAiCompanionVisualIdentityPreview(selectedId, view as "canonical" | "three-quarter" | "side"))).then((nextUrls) => {
    ...
    <main className="page-shell ai-companions-page">
      ...
      {detail ? <section className="ai-companion-workspace">
        ...
        <div className="ai-chat-card">
          ...
          {detail.visualIdentity?.status === "review" ? <div className="ai-disabled-note">{detail.companion.name}'s visual identity is awaiting the required consistency review. Photos will stay private until it passes. <button className="text-button" onClick={() => regenerateVisualIdentityMutation.mutate()} disabled={regenerateVisualIdentityMutation.isPending}>{regenerateVisualIdentityMutation.isPending ? "Resetting look..." : "Regenerate look"}</button>{regenerateVisualIdentityMutation.error ? <p className="form-error">{regenerateVisualIdentityMutation.error.message}</p> : null}{visualReferenceUrls.length === 3 ? <div className="ai-visual-reference-grid">{visualReferenceUrls.map((url, index) => <img src={url} alt={`${detail.companion.name} reference ${index + 1}`} key={url} />)}</div> : <p>Loading private references...</p>}</div> : null}
