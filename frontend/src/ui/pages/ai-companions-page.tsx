    ...
    <main className="page-shell ai-companions-page">
      ...
      {detail ? <section className="ai-companion-workspace">
        ...
        <div className="ai-chat-card">
          ...
          {detail.visualIdentity?.status === "review" ? <div className="ai-disabled-note">{detail.companion.name}'s visual identity is awaiting the required consistency review. Photos will stay private until it passes.{visualReferenceUrls.length === 3 ? <div className="ai-visual-reference-grid">{visualReferenceUrls.map((url, index) => <img src={url} alt={`${detail.companion.name} reference ${index + 1}`} key={url} />)}</div> : <p>Loading private references...</p>}</div> : null}
